...
for subj in SUBJ_LIST:
    #subj_PSD = []
    task_PSD, sham_PSD, epochs_tot = taskVSsham(subj, 'PSD_RT_AR_newEvent', 'epo_RT_AR_newEvent', FOLDERPATH)
    for c in CONDITIONS:
            cond1_tot = []
            cond2_tot = []
            ...
            for i in range(len(task_PSD)):
                epochs_id = str(i+1)
                cond1, cond2= split_by_condition(task_PSD[i], epochs_tot[epochs_id], None, None, condition = c, epochs_type = 'RT') #epochs_type could be 'Long' or 'RT'
            ...
            sham1, sham2 = split_by_condition(sham_PSD, epochs_tot['4'], c[0], c[1], condition = c)
            ...
            for t in range(len(t_list)):
                ...
                vmax = extreme
                vmin = -extreme
                reportname, reportpath = get_pareidolia_bids(FOLDERPATH, subj, 'pareidolia', '-', stage = 'fig_ttest_RT_1secAfter'+c)
